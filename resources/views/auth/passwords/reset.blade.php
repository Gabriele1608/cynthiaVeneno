        ...
        <div class="w-full">
            <section class="flex flex-col break-words bg-white sm:border-1 sm:rounded-md sm:shadow-lg">
                ...
                <header class="font-semibold bg-white text-black py-5 px-6 sm:py-6 sm:px-8 sm:rounded-t-md">
                    {{-- {{ __('Reset Password') }} --}} Restablecer la contraseña
                </header>

                <form class="w-full px-6 space-y-6 bg-black border border-white sm:px-10 sm:space-y-8" method="POST" action="{{ route('password.update') }}">
                    @csrf
